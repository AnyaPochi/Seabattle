...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game.status == 'in game':
            game.current_player.message.append("Стреляй, человек: ")
            game.draw()
            game.current_player.message.clear()
            shot_result = game.current_player.make_shot(game.next_player)

            if shot_result == 'miss':
                game.next_player.message.append('Игрок {}, промахнулся! '.format(game.current_player.name))
                game.next_player.message.append('Стреляй снова!'.format(game.next_player.name))
                game.switch_players()
                continue
        ...
        if game.status == 'game over':
            ...
            print('{} выиграл битву!'.format(game.current_player.name))
            break

    print('Битва окончена, но война продолжается!')
    input('')
